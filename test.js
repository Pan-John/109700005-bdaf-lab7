...
async function simulate(){
    ...
    console.log('init USDC Balance:                         ', withcommas(init_USDCbalance));
    ...
    await USDC.connect(aliceImpersonatedSigner).approve(CometAddress, 1000);
    await Comet.connect(aliceImpersonatedSigner).supply(USDCAddress, 1000);
    const balance_AfterAliceProvideLiquidity = await USDC.balanceOf(CometAddress);
    console.log('USDC Balance after Alice provide 1000 USDC:', withcommas(balance_AfterAliceProvideLiquidity));
    ...
    console.log('USDC Balance after Bob Withdraw all USDC:', withcommas(balance_AfteBobWithdrawAllUSDC) );
    ...
    try{ await Comet.connect(aliceImpersonatedSigner).withdraw(USDCAddress, 1000); }
